    ...
    qs =
      GRT.firstSuccessQ
        [ GRT.embedGitError gitDataFromGitQ,
          GRT.runGitInEnvDirQ [osstr|CABAL_MONITOR_HOME|] gitDataFromGitQ,
          gitDataFromEnvQ
        ]
...
--
-- - CABAL_MONITOR_MODIFIED: unix time like "1746055623"
-- - CABAL_MONITOR_HASH: long hash
-- - CABAL_MONITOR_SHORT_HASH: short hash
--
-- We have to convert the unix time into the intended format
-- YYYY-MM-DD.
gitDataFromEnvQ :: Q (Either GitRevError (OsString, OsString, OsString))
gitDataFromEnvQ = do
  let dateVar = [osstr|CABAL_MONITOR_MODIFIED|]
  d <- GRT.embedEnvError $ fmap (>>= displayUnixTime dateVar) (GRT.envValQ dateVar)
  h <- validateHash 40 <$> GRT.envValQ [osstr|CABAL_MONITOR_HASH|]
  sh <- validateHash 7 <$> GRT.envValQ [osstr|CABAL_MONITOR_SHORT_HASH|]
  pure $ liftA3 (,,) d h sh
  where
